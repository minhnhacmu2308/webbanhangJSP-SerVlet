          ...
          <div class="row"  id ="deleteBody">
            <div class="col-sm-12">
              <div class="card card-table">
                ...
                <%List<ProductModel> newsListP= (List<ProductModel>) request.getAttribute("listP"); %>
                <div class="card-body">
                  <table class="table table-striped table-hover table-fw-widget" id="table1">
                    ...
                    <tbody >
                    <% for(int i =0;i<newsListP.size();i++){ %>
                    <tr class="odd gradeX">
                            <td class="user-avatar cell-detail user-info"><img class="mt-0 mt-md-2 mt-lg-0" src="img/<%=newsListP.get(i).getProductPicture() %>" alt="Avatar">
                            <span> <input style="border-radius: 5px;border-color:#C0C0C0  " id="proName<%=i %>" name="proName" type="text"  value="<%= newsListP.get(i).getProductName() %>" onchange="aa(this,<%=i%>)">  <br>
                           Giá: <input id="price<%=i %>" name="price" type="text" style="border-radius: 5px;border-color:#C0C0C0 ;height: 20px;width: 90px;margin-top: 5px" value="<%=newsListP.get(i).getProductPrice()%>" onchange="aa(this,<%=i%>)">   
					     <td>
					     <textarea  id="proDes<%=i %>" name="proDes" rows="4" cols="50" onchange="aa(this,<%=i %>)"><%= newsListP.get(i).getProductDescription()%></textarea>
                           <input type="hidden"  value="<%= newsListP.get(i).getProductId()%>" id="proId<%=i %>" >       
                         </td>
                         
                       
                        
                        <td>
                         <a href="deleteproduct?idproduct=<%=newsListP.get(i).getProductId()%>" style="color: red" class="btn btn-space btn-outline-danger btn-space"><i style="color: red" class="icon icon-left mdi mdi-alert-circle"></i>Delete</a>
                        </td>
                    ...
                    <%} %>
                      ...
                      function aa(a,i) {
                    		var nameIdP = "proId"+i;
                    		var nameProP = "price"+i;
                    		var nameProN = "proName"+i;
                    		var nameProD = "proDes"+i;
                    	
                    		var proId = document.getElementById(nameIdP).value;
                    	
                    		if(a.name == "price"){
                    			
                    			var proName = document.getElementById(nameProN).value;
                    			var proDes = document.getElementById(nameProD).value;
                    			$.ajax({
                          		  url: "/WebsiteBanHang/editproduct",
                          		  type:"post",
                          		  data:{
                          			productId:proId,
                          			price:a.value,
                    		...
                    		}else if(a.name == "proName"){
                    			var price = document.getElementById(nameProP).value;
                    			var proDes = document.getElementById(nameProD).value;
                    			$.ajax({
                          		  url: "/WebsiteBanHang/editproduct",
                          		  type:"post",
